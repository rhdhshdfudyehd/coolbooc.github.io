...
$buyBtn.on('click', function() {
    ...
    const qrCode = new QRCodeStyling({
        ...
        type: "canvas",
        data: found[1],
        image: '/static/coolbooc/img/'+ selectedPayment.toLowerCase() +'.png',
        imageOptions: {
...
$(document).ready(function(){
  ...
  $('.filter-btn').on('click', function(){
    ...
    if(cat === 'all') {
      ...
      $('.product-row').hide();
      $('.product-row[data-category="'+cat+'"]').show();
    }
  });
  ...
  $('.faq-question').on('click', function(){
    ...
    $(this).find('.icon').text(expanded ? '−' : '+');
  });

  // 动态注入分类名称到 filter 按钮 (用于静态演示时显示中文, 实际生产由后台渲染, 但保留TP标签)
  // 仅当没有后台时模拟一下, 实际框架会输出。这里直接操作DOM模拟volist输出。
  // 但是不要删除volist标签，仅做功能增强。我们读取category JSON来创建按钮? 但为了保留TP语法, 我们假设后端已输出。
  // 如果纯粹静态演示，我们会把已有的volist内的按钮保留，不加干扰。
  // 为了安全，不覆盖TP标签。
});